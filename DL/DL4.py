    ...
    3. 신경망
        1) 인공 신경망 (Artifical Neural Network : ANN)
            - 생물학적인 신경망에서 영감을 받아서 만들어진 컴퓨팅 구조임.
                - 인간의 뇌 구조 모방
            - 장점
                - 학습이 가능함 (데이터가 주어지면 신경망은 배울수 있음)
                - 몇 개의 소자가 오동작 하더라도 전체적으로 큰 문제가 발생하지 않음

    4. 퍼셉트론(perceptron)
        1) 생물학적 뉴런을 모방하여 만든 인공신경망의 기본 단위
            이전 perceptron 과 연결 ==> inputs  weights  input_function  activation_function  output  ==> 다음 perceptron 으로 전달
        ...
        4) AND / OR 연산
            ...
            - 서론 다른 두 그룹을 구분하기 위해 선형 분류자 1개 필요
