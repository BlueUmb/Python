...
# astype() 메서드를 이용하여 데이터를 범주형(단어형)으로 변환
for category in categorical_columns:
    dataset[category] = dataset[category].astype('category')

# dataset[catrgory] => 넘파이 배열(숫자형)
...
'''
    컬럼의 고유 값수를 2로 나누는 것을 많이 사용
        - 예를 들어 price 컬럼은 4개의 고유 값을 갖기 때문에 4/2 = 2 (임베딩)
...
class Model(nn.Module):  # 클래스 형태로 구현되는 모델은 nn.Module을 상속받음
    ...
    def __init__(self, embedding_size, output_size, layers, p=0.4):
        ...
        for i in layers:  # 모델의 네트워크 계층 구축
            # 선형 계층은 입력 데이터에 선형 변환(입력과 가중치를 곱한 후 바이어스를 더한 결과)
            all_layers.append(nn.Linear(input_size, i))
            all_layers.append(nn.ReLU(inplace=True))  # 활성화 함수 사용
            all_layers.append(nn.BatchNorm1d(i))  # 배치 정규화 (batch normalization) 사용
            all_layers.append(nn.Dropout(p))  # Dropout : 과(한)적합 방지에 사용
            input_size = i

        all_layers.append(nn.Linear(layers[-1], output_size))
        self.layers = nn.Sequential(*all_layers)  # 레이어를 all_layers 리스트에 append 된 순서대로 적용시킴
...
'''
    Model 클래스의 객체를 생성
    객체를 생성하면서 범주형 컬럼의 임베딩 크기(embedding_size), 출력 크기(output_size), 
    은닉층의 뉴런(layers), 드롭아웃(p=0.4)을 전달
...
'''
    ...
    이 DL4 경우 데이터를 분류해야 하는 것으로 크로스 엔트로피(cross entropy) 손실함수를 사용
    옵티마이저는 아담(Adam)을 사용함

    lr(Learning Rate)   : 미분값을 얼마만큼 이동시킬 것인가를 결정함.
                          초기값이 크다면 초반에 loss값이 빠르게 줄것임. 나중에 가면 underfitting 이 발생 
'''
print("-" * 30)
...
for i in range(epochs):
    ...
    single_loss = loss_function(y_pred, train_outputs)  # 실제 값과 비교
    aggregated_loss.append(single_loss)

    if i % 25 == 1:
        print(f'epoch: {i:3} loss: {single_loss.item():10.8}')

    optimizer.zero_grad()  # 기울기 초기황
...
print(f'Loss : {loss:.8f}')  # 테스트 데이터셋에 대한 손실값을 출력
...
# 신경망에서 필요한 모든 파라미터를 무작위로 선택했다는 것을 감안할때 75%의 정확도는 나쁘지 않음
