'''
    ...
    2. 텐서플로 (TensorFlow)
        - 기계 학습 프레임워크(framework)중 하나임.
            - 텐서(tensor)를 Numpy 배열처럼 사용할수 있음.
            - 데이터 흐름 그래프 (data flow graph)를 사용하여 데이터의 수치연산을 하는 오픈 소스 소프트웨어 프레임워크
            - 넘파이 같은 다양한 라이브러리를 묶어 놓은 패키리자고도 할 수 있음
        - Tensorflow를 사용하면, GPU 연동을 통해 효율적으로 딥러닝 모델을 학습할수 있음.
    ...
    4. 텐서플로 특징 및 장점
        ...
        5) 텐서플로 2.x 부터는 데이터를 학습하고 예측하는 과정이 매우 단순해졌음
...
'''
    * GPU 사용 여부 체크하기 
        - GPU를 사용하면 텐서플로우에서 딥러닝 모델을 더욱 효과적으로 학습할수 있음
...
tf.debugging.set_log_device_placement(False)    # False 가 기본값
print(c)
